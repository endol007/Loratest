...
socket.on('data', function (msg) {
  ...
  var solar = Number(msg.solar);
  var seqn = msg.seqn;
  var interval;

  nodes[eui].interval = interval = nodes[eui].last_in ?
    (new Date(msg.timestamp) - new Date(nodes[eui].last_in)) / 1000 : 0;
  nodes[eui].data[datr].push({ rssi: rssi, lsnr: lsnr, seqn: seqn, solar: solar, interval: interval });
...
function getMaxMinAvg(data) {
  ...
  for (; i < length; i++) {
    ...
    if (data[i].solar > solar.max) solar.max = data[i].solar;
    if (data[i].solar < solar.min) solar.min = data[i].solar;
  ...
  solar.avg = Number((solar.avg / div).toFixed(2));
